...
ID3D12Resource* CreareDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height) {
	// 生成するリソースの設定
...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	ID3D12Resource* depthStencilResource = CreareDepthStencilTextureResource(device, kClientWidth, kClientHeight);
	...
	// 頂点関数
	ID3D12Resource* vertexResource = CreateBufferResouce(device, sizeof(VertexData) * startIndex);
	...
	DrawSphere(vertexData, kSphereSubdivision);
	// ビューポート
	...
	*transformationMatrixDataSprite = m4.MakeIdentity4x4();
	...
	// 2. リソースをマップして、書き込み先のポインタアドレスを取得
	HRESULT hrLight = directionalLightResource->Map(0, nullptr, reinterpret_cast<void**>(&directionalLightData));
	assert(SUCCEEDED(hrLight));
